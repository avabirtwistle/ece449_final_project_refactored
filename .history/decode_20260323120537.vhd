...
begin
    ...
    -- format A Case
    -- For A0 format, this is NOP 
    -- the PC needs to be sent the NOP for this 
    -- should this be sent in the ID/EX register and manage in execute /
    pc_mode <= PC_NOP;

    -- for A1 format this is 
    ra<= instruction(8 downto 6);
    rb<= instruction(5 downto 3);
    rc<= instruction(2 downto 0);

    -- for A2 format this is 
    ra <= instruction(8 downto 6);
    shift_amount <= instruction(3 downto 0);

    -- for A3 format this is 
    ra<= instruction(8 downto 6);
